...
class SuggestedTimeSlotsView(APIView):
    ...
    def post(self, request):
        try:
            ...
            user_time_preferences = get_user_timing_preferences_for_users(
                user_ids=user_ids
            )
            ...
            condition_assert(
                len(user_ids) == len(calendar_data),
                "Number of user ids and calendar data do not match",
            )
            condition_assert(
                truth=all(user_id in calendar_data for user_id in user_ids),
            ...
            for user_id in user_ids:
                busy_slots.append(
                    convert_busy_slots_to_common_timezone(
                        calendar_data[user_id]["calendars"]["primary"]["busy"]
                    )
                )
            ...
            return Response(
                get_response_dict(
                    message="Suggested time slots retrieved successfully", data=slots
                ),
