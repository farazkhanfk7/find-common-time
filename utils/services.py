...
def get_user_timing_preferences_for_users(user_ids: List[int]) -> List[UserTimingPreference]:
    """
    ...
    for user_id in user_ids:
        try:
            ...
            condition_assert(utp_obj.day_start_time is not None and utp_obj.day_end_time is not None and utp_obj.timezone is not None, f"UserTimingPreference for user_id: {user_id} does not have all required fields")
            ...
            raise ConditionFailedException(f"UserTimingPreference does not exist for user with id: {user_id}")
